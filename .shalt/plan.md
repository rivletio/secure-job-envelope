...
Buyer seals a quoteable traveler (L0) → sellers bind quotes to `traveler_hash` (L1) → buyer awards + ops + ship-to (L2). L3 as-built is reserved.
...
`.traveler.enc` for harvest-now-decrypt-later. Design lives in `docs/ENVELOPE-DRAFT.md`. No scenarios yet — do not pretend this is in 0.0.1.

## Current state

- Spec `opentraveler/0.0.1` is written (`docs/SPEC.md`, JSON Schema, 15 Gherkin features / 88 scenarios).
