@epic:quote
Feature: Bind a structured quote to a packet hash
  A seller prices quantity breaks against the current packet_hash.
  ...
  @rid:S-ed931467
  Scenario: Seller opens quote dialog on an L0 packet
    When the seller opens "/p/pkt_wldcart12"
    And the seller clicks "Quote this hash"
    Then the quote dialog shows packet_hash_quoted equal to the packet hash
    And the dialog lists unit inputs for qty breaks 4, 12, and 24
    And the seller identity is "Lakeshore Turning"

  @rid:S-3d848957
  Scenario: Bind quote promotes packet from L0 to L1
    When the seller opens "/p/pkt_wldcart12"
    And the seller clicks "Quote this hash"
    ...
    Then packet "pkt_wldcart12" is level "L1"
    ...
    And the local audit contains a "quote" event for "pkt_wldcart12"
    And the quote button on the packet reads "Quoted" and is disabled
  ...
  Scenario: Cannot bind a quote with no priced lines
    ...
    And packet "pkt_wldcart12" remains level "L0"

  @rid:S-21f49a9e
  Scenario: Same seller cannot place a second quote on the same revision
    Given shop "org_lakeshore" already quoted "pkt_wldcart12" on the current hash
    When the seller opens "/p/pkt_wldcart12"
    Then the quote button reads "Quoted" and is disabled

  @rid:S-39a5c4ae
  Scenario: Quote on a Draft packet is not offered
    Given a draft packet "pkt_draft01" missing material spec
    And the role is "seller"
    When the seller opens "/p/pkt_draft01"
