@epic:conformance
Feature: Packet conformance levels
  ...
  @rid:S-ef846a17
  Scenario Outline: Missing quoteable field keeps the packet at Draft
    Given a packet that is otherwise quoteable
  ...
  Scenario: Quoteable L0 has material and qty but no bound quote
    Given packet "pkt_wldcart12" from the seed desk
    When the conformance level is computed
    Then the level code is "L0"
    And the level name is "Quoteable"
    And missing includes "bound quote (packet_hash_quoted)"

  @rid:S-ec689298
  Scenario: Awardable L1 has at least one hash-bound quote
    Given packet "pkt_nlbrk4410" from the seed desk
    When the conformance level is computed
  ...
  Scenario: Executable L2 has award, ops, and ship-to
    Given packet "pkt_smenc1601" from the seed desk
    When the conformance level is computed
  ...
  @rid:S-a3ab4c9d
  Scenario: Bound quotes must match the current packet hash
    Given packet "pkt_nlbrk4410" with two quotes on the current hash
    When the part material spec changes and revision is not yet bumped via amend
    And quotes still carry the previous packet_hash_quoted
    Then those quotes are stale
    And bound quote count is 0 for the new hash

  @rid:S-9b6c36d9
  Scenario: Non-ITAR seller quote is excluded from bound set on an ITAR packet
    Given an ITAR packet whose current hash is H
