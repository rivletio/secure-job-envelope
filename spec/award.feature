@epic:award
Feature: Award a bound quote and make the traveler executable
  ...
  Scenario: Buyer awards a live quote with ship-to and ops
    ...
    And the local audit contains an "award" event for "tvl_award01"
    And a success toast says "Traveler is L2 executable."
  ...
  Scenario: Award requires a complete ship-to
    ...
    And traveler "tvl_award01" remains level "L1"
  ...
  Scenario: Cannot award a quote with no unit price at target qty
    Given L1 traveler "tvl_award01" with target qty 50
  ...
  Scenario: Seller role does not show Award buttons
    ...
    When the seller opens "/p/tvl_nlbrk4410"
    Then no "Award" button is shown on bound quotes

  @rid:S-7cd6dc95
  Scenario: Award is refused when the traveler is already executable
    Given L2 traveler "tvl_smenc1601" on the desk
    When the store award is invoked again for its existing quote
    Then the operation fails with "Traveler is already executable."
