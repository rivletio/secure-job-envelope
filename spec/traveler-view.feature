@epic:traveler
Feature: View a traveler on the desk
  Opening a traveler shows the quoteable body, bound quotes, hash, and level checklist.

  Background:
    Given the desk is seeded with the demo travelers

  @rid:S-e321229f
  Scenario: Missing traveler id shows a not-on-desk message
    When the user opens "/p/tvl_doesnotexist"
    Then the page shows "No traveler tvl_doesnotexist on this desk."
  ...
  Scenario: L1 bracket lists two quotes bound to the current hash
    When the user opens "/p/tvl_nlbrk4410"
  ...
  Scenario: L2 enclosure shows executable ship-to and ops
    When the user opens "/p/tvl_smenc1601"
  ...
  Scenario: Sidebar checklist advances with level
    ...
    And the checklist next step is "bound quote (traveler_hash_quoted)"
    When the user opens "/p/tvl_smenc1601"
