...
export const quoteSchema = z
  .object({
    quote_id: z.string().regex(PACKET_ID_RE),
    seller: orgSchema,
    packet_hash_quoted: z.string().regex(HASH_RE),
    created_at: isoDt,
    valid_until: isoDt,
...
export const packetSchema = z
  .object({
    spec: z.literal(PACKET_SPEC),
    packet_id: z.string().regex(PACKET_ID_RE),
  ...
  .strict()
  .superRefine((p, ctx) => {
    if (JSON.stringify(p).length > MAX_PACKET_JSON_BYTES) {
      ctx.addIssue({ code: "custom", message: "packet exceeds 512 KiB" });
    }
    if (p.itar) {
      for (const [i, q] of (p.quotes ?? []).entries()) {
        if (q.seller.itar !== true) {
          ctx.addIssue({
            ...
            message: "ITAR packet cannot carry a quote from a non-ITAR seller",
          });
        }
      }
    }
