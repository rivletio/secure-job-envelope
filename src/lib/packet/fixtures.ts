...
import type { Packet, Quote, QuoteableBody } from "./types.ts";
import { PACKET_SPEC } from "./types.ts";

function withBoundQuotes(packet: Omit<Packet, "quotes"> & { quotes?: Quote[] }): Packet {
  const base: Packet = { ...packet, quotes: [] };
  const hash = packetHash(base);
  return {
    ...base,
    quotes: (packet.quotes ?? []).map((q) => ({ ...q, packet_hash_quoted: hash })),
  };
}
...
export function seedPackets(): Packet[] {
  const bracket = withBoundQuotes({
    spec: PACKET_SPEC,
    packet_id: "pkt_nlbrk4410",
    revision: 1,
  ...
  const weldment: Packet = {
    spec: PACKET_SPEC,
    packet_id: "pkt_wldcart12",
    revision: 1,
...
/** Golden quoteable body — must match crates/rivlet-packet tests. */
