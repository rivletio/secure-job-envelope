...
pub const SPEC: &str = "opentraveler/0.0.1";
pub const MEDIA_TYPE: &str = "application/vnd.opentraveler+json";
...
pub struct Quote {
    pub quote_id: String,
    pub seller: Org,
    pub traveler_hash_quoted: String,
...
pub fn parse_traveler(json: &str) -> Result<Traveler, Error> {
    if json.len() > 512 * 1024 {
        return Err(Error::Invalid("traveler json exceeds 512 KiB".into()));
    }
    let traveler: Traveler = serde_json::from_str(json)?;
...
fn write_number(out: &mut String, n: &serde_json::Number) -> Result<(), Error> {
    if let Some(i) = n.as_i64() {
        if i.unsigned_abs() > MAX_SAFE_INTEGER as u64 {
            return Err(Error::Invalid(
                "integer exceeds 2^53-1 and is not canonical in opentraveler/0.0.1".into(),
            ));
        }
    ...
    if f.abs() > MAX_SAFE_INTEGER {
        return Err(Error::Invalid(
            "number exceeds 2^53-1 and is not canonical in opentraveler/0.0.1".into(),
        ));
    }
    if f.fract() == 0.0 {
    ...
    if f.abs() < 1e-5 {
        return Err(Error::Invalid(
            "non-integer number below 1e-5 is outside the canonical fixed-notation range of opentraveler/0.0.1".into(),
        ));
    }
    let rendered = serde_json::to_string(&f).expect("finite f64");
    if rendered.contains('e') || rendered.contains('E') {
        return Err(Error::Invalid(
            "number outside the canonical fixed-notation range of opentraveler/0.0.1".into(),
        ));
    }
...
pub fn traveler_hash(traveler: &Traveler) -> Result<String, Error> {
    hash_quoteable(&quoteable_from(traveler))
}

pub fn is_quoteable(traveler: &Traveler) -> bool {
    !traveler.buyer.name.trim().is_empty()
        && !traveler.part.family.trim().is_empty()
        && !traveler.part.part_number.trim().is_empty()
        && !traveler.part.material.spec.trim().is_empty()
        && traveler.part.qty.target >= 1
}

/// A traveler whose quoteable body cannot hash (out-of-range numbers) can have
/// no bound quotes: binding is defined by the hash.
pub fn bound_quotes(traveler: &Traveler) -> Vec<&Quote> {
    let Ok(hash) = traveler_hash(traveler) else {
        return Vec::new();
    };
    traveler
        .quotes
        ...
        .unwrap_or(&[])
        .iter()
        .filter(|q| q.traveler_hash_quoted == hash && quote_ok(q, traveler))
        .collect()
}
...
fn quote_ok(q: &Quote, traveler: &Traveler) -> bool {
    ...
    let hash_ok = q.traveler_hash_quoted.starts_with("sha384:")
...
pub fn level(traveler: &Traveler) -> Level {
    if !is_quoteable(traveler) {
        return Level {
            n: 0,
            code: "D",
            name: "Draft",
    ...
    let bound = bound_quotes(traveler);
    if bound.is_empty() {
        return Level {
            n: 0,
            code: "L0",
