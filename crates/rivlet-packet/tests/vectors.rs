...
//! implementation (src/lib/packet/vectors.test.ts); agreement across both
//! is the cross-implementation proof. See conformance/README.md.

use rivlet_packet::{canonical_json, level, packet_hash, parse_packet};
...
#[test]
fn packet_vectors_match_expected_hash_and_level() {
    let dir = conformance_dir().join("packets");
    ...
    for (file, exp) in map {
        ...
        let packet = parse_packet(&json).unwrap_or_else(|e| panic!("{file}: refused: {e}"));
        assert_eq!(
            packet_hash(&packet).unwrap(),
            exp["packet_hash"].as_str().unwrap(),
            "hash: {file}"
        );
        assert_eq!(level(&packet).code, exp["level"].as_str().unwrap(), "level: {file}");
    }
}

#[test]
fn reject_vectors_are_refused_at_parse() {
    let dir = conformance_dir().join("packets/reject");
    let mut count = 0;
    for entry in fs::read_dir(&dir).unwrap() {
        let path = entry.unwrap().path();
        let json = fs::read_to_string(&path).unwrap();
        assert!(
            parse_packet(&json).is_err(),
            "must refuse: {}",
            path.display()
        );
        count += 1;
